...
// NOTE: all figures below are generic DemoCo-style fixtures. Never put real
// personal compensation numbers in committed test data.
function startupOffer(startupEquity: TStartupEquity, startDate = '2024-08-01'): TOffer {
  return {
...
describe('startup equity feeds computeOffer (regression: equity-over-4-years showed $0)', () => {
  it('includes startup option value in yearly stock rows', () => {
    const offer = startupOffer(block({
      optionGrants: [
        { label: 'new hire', quantity: 2000, strike: 25, fmvAtGrant: 90, vestYears: 4, cliffMonths: 12 },
      ],
    }));
    const rows = computeOffer(offer);
    const total = rows.reduce((a, r) => a + r.stock, 0);
    // ~2000 * (100 - 25) fully vested over 4 years
  ...
  it('includes double-trigger RSU value in yearly stock rows', () => {
    const offer = startupOffer(block({
      rsuGrants: [
        { label: 'rsus', shares: 1000, fmvAtGrant: 100, doubleTrigger: true, vestYears: 4 },
      ],
    }));
    const rows = computeOffer(offer);
    const total = rows.reduce((a, r) => a + r.stock, 0);
  ...
  it('returns 0 stock when the block is disabled or absent', () => {
    const disabled = startupOffer(block({
      enabled: false,
      rsuGrants: [{ label: 'rsus', shares: 1000, fmvAtGrant: 100, doubleTrigger: true, vestYears: 4 }],
    }));
  ...
  it('honors grantStartDate for refresh grants', () => {
    const offer = startupOffer(block({
      rsuGrants: [
        { label: 'refresh', shares: 200, fmvAtGrant: 100, doubleTrigger: true, vestYears: 2, grantStartDate: '2025-08-01' },
      ],
    }));
    const rows = computeOffer(offer);
  ...
  it('adds startup stock on top of public equityGrants without double counting', () => {
    const withStartup = startupOffer(block({
      rsuGrants: [{ label: 'rsus', shares: 200, fmvAtGrant: 100, doubleTrigger: false, vestYears: 4 }],
    }));
